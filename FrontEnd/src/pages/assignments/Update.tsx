import {
  ...
  import { AssignmentModel, AssignmentState } from "../../models/Assignment";
  ...
  export function UpdateAssignment() {
    ...
    useEffect(() => {
        (async () => {
          let assignment = await service.getAssignment(assignmentId);
          let user = await userService.getUser(assignment.assignedToUserId);
          let asset = await assetService.getAsset(assignment.assetId)
    ...
    const onFinish = (data: AssignmentModel) => {
      (async () => {
        await service.update(assignmentId,data);
